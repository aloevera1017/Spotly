...
def geocode_address(address):
    ...
    try:
        geolocator = Nominatim(user_agent="nyc_parking_predictor")
...
# Title
st.markdown('<h1 class="main-header">🅿️ NYC Parking Predictor - Individual Spaces</h1>', unsafe_allow_html=True)
...
if 'space_predictions' in st.session_state:
    ...
    with col_map:
        st.markdown("### 🗺️ Individual Parking Spaces Map")
    ...
    with col_list:
        ...
        for idx, row in location_summary.head(20).iterrows():
            color = "🟢" if row['Availability %'] >= 50 else ("🟠" if row['Availability %'] >= 25 else "🔴")
            with st.expander(f"{color} {row['Address'][:30]}... - {row['Available']}/{row['Total']} available"):
...
<div style='text-align: center; color: #64748b; padding: 1rem;'>
    <p><b>NYC Parking Predictor - Individual Space Tracking</b> 🅿️</p>
    <p>Real-time prediction for each parking space | Datathon 2024</p>
</div>
""", unsafe_allow_html=True)
